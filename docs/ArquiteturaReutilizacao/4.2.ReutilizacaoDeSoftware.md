...
Um exemplo claro de instanciação ocorre no componente 'PodcastStudioCard'. Esse componente é utilizado repetidamente para exibir diferentes podcasts, mas é instanciado com diferentes objetos 'podcast' e índices via props:

Internamente, o componente utiliza esses dados para preencher campos como 'titulo', 'descricao', 'tags' e 'reproducoes', permitindo que uma única definição estrutural gere múltiplas instâncias visuais distintas.
